...
- 写入前会再次比较源文件字节/哈希，发现并发变化就删除本次临时产物并拒绝覆盖。检查与原子替换之间仍有极小的无锁竞态窗口，因此不是严格 CAS。
...
本项目是独立的脚本式重实现，不复制或修改参考仓库。它保留核心 Local State 变换，但采用先全量验证、原子替换、dry-run 无副作用及按已知路径停止进程；按当前需求不生成备份。会产生托管提示的“禁用 AI 下载”也从参考实现的侵入式默认开启改成**明确选择、默认关闭**。macOS/Linux 与 Windows 分别使用平台原生脚本，便于审阅，无需 Go 构建产物。
